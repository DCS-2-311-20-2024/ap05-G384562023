...
function init() {
  ...
  const sphre = new THREE.Mesh(sphereGeometry, sphereMaterial);

  // 球にテクスチャを登録
  sphereMaterial.map = texture2;

  // 球の位置
  sphre.position.set(2.5, 2, -1);
  // 球は影を作る
  sphre.castShadow = true;
  // シーンに球を加える
  scene.add(sphre);
  // 平面の作成
  ...
  vender01.children.forEach((child) => {
  ...
  vender02.children.forEach((child) => {
    child.castShadow = true;
    child.receiveShadow = true;
  });
  scene.add(vender02);
  ...
  function render() {
    ...
    sphre.rotation.y += 0.01 * controls.rotation;
    // 描画
